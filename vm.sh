...
CLI_HDD="-device virtio-blk-pci,scsi=off,bus=pci.0,addr=0x5,drive=drive-virtio-disk0,id=virtio-disk0,bootindex=1 -drive file=${ROOTFS},if=none,id=drive-virtio-disk0,format=qcow2"
...
if [ "$KERNEL" != "" ]; then
	CLI_BOOT="-append 'root=$ROOT ro console=ttyS0 debug rdinit=/bin/init'"
	CLI_KERNEL="-kernel $KERNEL"
	pr_info "[INFO] Linux kernel to load: $KERNEL"

	if [ "$INITRD" != "" ]; then
		CLI_INITRD="-initrd $INITRD"
		pr_info "[INFO] Initramfs to load: $INITRD"
	fi
else
